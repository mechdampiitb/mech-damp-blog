...
export async function PATCH(
  ...
  // 1. Auth check — first thing in the function, before touching any data
  ...
  // 2. Validate the incoming status + comment, before doing anything with them
  if (!VALID_STATUSES.includes(status)) {
    return NextResponse.json({ error: "Invalid status value" }, { status: 400 });
  }
  if ((status === "rejected" || status === "changes_requested") && !adminComment?.trim()) {
    return NextResponse.json({ error: "adminComment is required when rejecting or requesting changes" }, { status: 400 });
  }

  // 3. Everything below this line is your existing logic, unchanged
